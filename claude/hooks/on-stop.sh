...
if [[ "$CWD" == */agents/* ]] && command -v maw >/dev/null 2>&1; then
  ...
  if [ -n "$SESSION" ]; then
    ...
    REPO=$(echo "$CWD" | grep -oP 'your-org/\K[^/]+(?=/agents)' || echo "unknown")
    ...
    if [ -n "$BRANCH" ]; then
      # Check for open PRs
      PR_NUMS=$(gh pr list --repo "your-org/${REPO}" --head "$BRANCH" --state open --json number -q '.[].number' 2>/dev/null | tr '\n' ' ')
...
# ─── ARRA REINDEX ───────────────────────────────────────────────────
ARRA_DIR="$HOME/ghq/github.com/your-org/arra-oracle-v3"
