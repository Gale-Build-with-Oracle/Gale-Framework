"""
Solution Lab PDF Template Helper
Creates PDF documents with Solution Lab branding (logo header, footer attribution)
"""
...
SLAB_PRIMARY = HexColor('#D4652C')
SLAB_SECONDARY = HexColor('#333333')
SLAB_ACCENT = HexColor('#E88B5A')


def create_slab_header_footer(canvas, doc):
    """Draw Solution Lab header and footer on each page"""
    canvas.saveState()
    ...
    canvas.setFillColor(SLAB_SECONDARY)
    canvas.drawString(0.75*inch, doc.pagesize[1] - 0.55*inch, SLAB_HEADER_TEXT)

    # Footer attribution
    canvas.setFont("Helvetica", 9)
    canvas.setFillColor(HexColor('#666666'))
    canvas.drawCentredString(doc.pagesize[0]/2, 0.5*inch, SLAB_HEADER_TEXT)
...
def create_slab_pdf(filename, title=None, content=None, pagesize=letter):
    """
    Create a basic Solution Lab branded PDF document
    ...
    doc.build(story, onFirstPage=create_slab_header_footer, onLaterPages=create_slab_header_footer)
    return filename


def create_slab_test_report(filename, data=None):
    """
    ...
    prepared_by = data.get('prepared_by', 'Solution Lab')
    ...
    if test_cases:
        ...
        table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), SLAB_PRIMARY),
    ...
    create_slab_pdf(filename, content=story)
    return filename
...
def create_slab_meeting_minutes(filename, data=None):
    """
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: python slab_pdf_template.py [test-report|db-report|meeting]")
        sys.exit(1)

    command = sys.argv[1]

    if command == 'test-report':
        create_slab_test_report('test-report.pdf', {
        ...
        create_slab_db_report('db-report.pdf', [
        ...
        create_slab_meeting_minutes('meeting-minutes.pdf', {
            ...
            'action_items': [
                {'assignee': 'Solution Lab', 'task': 'Setup repository', 'due_date': '2026-03-20'}
            ]
        })
