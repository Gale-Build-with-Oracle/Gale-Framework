"""
Solution Lab XLSX Template Helper
Creates Excel spreadsheets with Solution Lab branding (logo header, footer attribution)
"""
...
LOGO_PATH = os.path.join(os.path.dirname(__file__), '..', 'solution-lab-logo.png')
SLAB_HEADER_TEXT = "Solution Lab - Innovation & Technology"

# Solution Lab Brand Colors
SLAB_PRIMARY = 'D4652C'
SLAB_SECONDARY = '333333'
SLAB_ACCENT = 'E88B5A'


def create_slab_workbook(sheet_name="Sheet1"):
    """
    Create a new workbook with Solution Lab branding
    ...
    # Set print header and footer with the canonical brand line
...
def apply_header_style(cell):
    """Apply Solution Lab header style to a cell"""
    cell.font = Font(bold=True, color="FFFFFF", size=11)
    cell.fill = PatternFill(start_color=SLAB_PRIMARY, end_color=SLAB_PRIMARY, fill_type="solid")
    cell.alignment = Alignment(horizontal='center', vertical='center')


def apply_data_style(cell, bold=False, color=None, wrap=True):
    """Apply Solution Lab data style to a cell"""
...
def create_slab_uat_template(filename, project_name="Project", test_cases=None):
    """
    ...
    """
    wb, ws = create_slab_workbook(f"UAT - {project_name}")

    # Title row (below logo)
    ws['A5'] = f"UAT Test Cases - {project_name}"
    ws['A5'].font = Font(bold=True, size=16, color=SLAB_PRIMARY)
...
def create_slab_test_summary(filename, data=None):
    """
    ...
    wb, ws = create_slab_workbook("Test Summary")
    ...
    ws['A5'].font = Font(bold=True, size=18, color=SLAB_PRIMARY)
    ...
    ws['A11'].font = Font(bold=True, size=14, color=SLAB_SECONDARY)
    ...
    # Test results table
    ws['A17'] = "Test Results"
    ws['A17'].font = Font(bold=True, size=14, color=SLAB_SECONDARY)
...
def create_slab_db_report(filename, transactions=None, title="Database Transaction Report"):
    """
    ...
    wb, ws = create_slab_workbook("DB Report")
...
def create_slab_data_table(filename, sheet_name="Data", headers=None, rows=None, title=None):
    """
    Create a generic data table spreadsheet with Solution Lab branding
    ...
    """
    wb, ws = create_slab_workbook(sheet_name)

    start_row = 5

    if title:
        ws[f'A{start_row}'] = title
        ws[f'A{start_row}'].font = Font(bold=True, size=16, color=SLAB_PRIMARY)
        start_row += 2
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: python slab_xlsx_template.py [uat|test-summary|db-report|data-table]")
        sys.exit(1)

    command = sys.argv[1]

    if command == 'uat':
        create_slab_uat_template('uat-template.xlsx', 'Sample Project', [
            {
                'id': 'TC001',
                'scenario': 'User Login',
        ...
        create_slab_test_summary('test-summary.xlsx', {
            'project_name': 'Solution Lab System',
            'test_cases': [
        ...
        create_slab_db_report('db-report.xlsx', [
            {'id': 'TX001', 'table': 'Orders', 'action': 'INSERT', 'status': 'Success', 'timestamp': '2026-03-12 10:00:00'},
            {'id': 'TX002', 'table': 'Customers', 'action': 'UPDATE', 'status': 'Success', 'timestamp': '2026-03-12 10:05:00'}
        ])
        print("Created db-report.xlsx")

    elif command == 'data-table':
        create_slab_data_table(
            'data-table.xlsx',
