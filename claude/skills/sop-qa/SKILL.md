...
cat ~/ghq/github.com/your-org/<PROJECT>/TASK-*.md 2>/dev/null
...
```bash
# Export creds locally first (never commit literals): export Acme_TEST_USER=... Acme_TEST_PASSWORD=...
TOKEN=$(curl -s -X POST http://localhost:<port>/api/auth/login \
  -H "Content-Type: application/json" \
  -d "{\"username\":\"$Acme_TEST_USER\",\"password\":\"$Acme_TEST_PASSWORD\"}" | jq -r '.token // .access_token // empty')
...
### 7.1 Schema Checks (via example-sql MCP)
...
Validate that the files changed in this PR are consistent with the requirements cited in its `REQ:` line. A PR claiming `REQ: REQ-example-001` (label printing) but modifying auth middleware is a scope leak — either the REQ line is wrong or the change is out of scope.
...
## Acme Legacy Exceptions (Wind-Approved)

- Hard DELETE of inventory records is ALLOWED (example projects)
- FDA 21 CFR Part 11 does NOT apply (Acme = food manufacturer)
- ALCOA+ Enduring: BinTransfer audit trail is sufficient for example projects
