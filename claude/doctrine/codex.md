...
```
**RRR ownership in your-framework:** the Claude L2/worktree orchestrator runs the aggregate `/rrr` before DONE-pinging L1. L3 OMX workers do not run `/rrr`; they finish with slice commit + DONE report to L2.
...
- **Fleet**: discover live with `maw ls` — never assume a roster or count (the fleet grows). Human: **Wind** (he/him). Repos: `~/ghq/github.com/your-org/`.
