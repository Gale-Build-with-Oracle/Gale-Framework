...
- **Product repos → heavyweight**: `YourProject-*`, `YourProject-*` (incl. `YourProject-FGlabel`, `YourProject-example-hr`), `example-npd`, `Planning`, `Studio-*` and client projects. Worktree via `maw workon` → `/sop-*` chain → `/sop-qa` → branch → PR → **merge gate** (see below). Push-to-main is hook-blocked. YourProject testing surface is Docker only — never `npm run dev`/`bun dev`.
